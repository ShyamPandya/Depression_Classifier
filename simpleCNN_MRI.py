...
#import input_3Dimage as inp
import tensorflow as tf
import csv
import testing as input_data
from sklearn.cross_validation import train_test_split
sess = tf.InteractiveSession()



# Placeholders (MNIST image:256x256x176 pixels=1,15,34,336, label=2)
x = tf.placeholder(tf.float32, shape=[None, width*height*depth]) # [None, 256x256x176]
y_ = tf.placeholder(tf.int64, shape=[None, nLabel])  # [None, 2]
...
W_fc1 = weight_variable([20*20*13*64, 1024])  # [7*7*64, 1024]
b_fc1 = bias_variable([1024]) # [1024]]

h_pool2_flat = tf.reshape(h_pool2, [-1,20*20*13*64])  # -> output image: [-1, 7*7*64] = 3136
...
for i in range(1):
    ...
    #batch = inp.get_data_jpeg(sess,3,filenames)
    filenames = input_data.get_filenames()
    X_neg, X_pos, y_neg, y_pos = input_data.get_data(1,filenames)

    train_X_pos, test_X_pos = train_test_split(X_pos, train_size=0.8)
    train_y_pos, test_y_pos = train_test_split(y_pos, train_size=0.8)
    train_X_neg, test_X_neg = train_test_split(X_neg, train_size=0.8)
    train_y_neg, test_y_neg = train_test_split(y_neg, train_size=0.8)

    print('Data fetched\n')
    print('Training samples: %d\nValidation samples: %d\n')
    train_accuracy = accuracy.eval(feed_dict={x:train_X_pos, y_: train_y_pos, keep_prob: 0.8})
    print("step %d, training accuracy %g"%(i, train_accuracy))
    train_step.run(feed_dict={x: train_X_pos, y_: train_y_pos, keep_prob: 0.5})

#Evaulate our accuracy on the test data
#testset = inp.get_data_jpeg(sess,'test',30)
